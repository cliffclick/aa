  ...
  @Override public Node ideal_grow() {
    ...
    if( _t instanceof TypeFunSig ) {
      ...
      try(GVNGCM.Build<Node> X = Env.GVN.new Build<>()) {
        ...
        args[DSP_IDX] = X.xform(new ParmNode(DSP_IDX,"disp",fun,(ConNode)Node.con(TypeMemPtr.DISP_SIMPLE),_error_parse));
        for( int i=ARG_IDX; i<sig.nargs(); i++ )  // 1 is memory, 2 is display.
          // All the parms; types in the function signature
          args[i] = X.xform(new ParmNode(i,"arg"+i,fun,(ConNode)Node.con(Type.SCALAR),_error_parse));
