...
public class NewObjNode extends NewNode<TypeStruct> {
  ...
  public Node get(String name) {
    //int idx = _ts.fld_find(name);  assert idx >= 0; return fld(idx);
    // alignment between field names and node offsets
    throw unimpl();
  }
  public boolean exists(String name) { return _ts.fld_find(name)!=null; }
  public boolean is_mutable(String name) {
    return _ts.fld_find(name)._access==Access.RW;
  }
  ...
  public void update( String tok, Access mutable, Node val ) {
    //update(_ts.fld_find(tok),mutable,val);
    throw unimpl();
  }
  ...
  public FunPtrNode add_fun( Parse bad, String name, FunPtrNode ptr ) {
    ...
    //}
    //return ptr;
    // Building a mapping between string and node offset
    throw unimpl();
  }
  ...
  public void promote_forward( NewObjNode parent ) {
    ...
    for( int i=0; i<ts.len(); i++ ) {
      ...
      if( n != null && n.is_forward_ref() ) {
        ...
        // TODO: mapping from field number and offset
        throw unimpl();
      }
    }
  }
  ...
  @Override TypeObj valueobj() {
    ...
    // TODO makes mapping between node inputs and field names
    throw unimpl();
  }
