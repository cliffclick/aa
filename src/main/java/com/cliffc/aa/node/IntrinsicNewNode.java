...
public abstract class IntrinsicNewNode extends Node {
  ...
  final int _alias;             // Main alias
  final TypeStruct _targs;      // Arguments
  IntrinsicNewNode( String name, int alias, TypeStruct targs ) {
    super(OP_LIBCALL);
    _name = name;
    _alias = alias;
    _targs = targs;
  ...
  public FunPtrNode as_fun( GVNGCM gvn ) {
    ...
    Node nnn = gvn.xform(new NewStrNode(_alias,TypeStr.STR,fun,rez));
    Node ptr = gvn.xform(new  ProjNode(nnn,1)); // ProjNode before OProj, so NewNode does not die for no ptr usage
...
class ConvertI64Str extends IntrinsicNewNode {
  ConvertI64Str() { super("str",BitsAlias.I64,TypeStruct.INT64); }
...
class ConvertF64Str extends IntrinsicNewNode {
  ConvertF64Str() { super("str",BitsAlias.F64,TypeStruct.FLT64); }
  @Override public Node ideal(GVNGCM gvn) { return null; }
  @Override public Type value(GVNGCM gvn) {
    Type t = gvn.type(in(2));
    if( t.above_center() ) return all_type().dual();
...
class AddStrStr extends IntrinsicNewNode {
  ...
  //
  AddStrStr() { super("+", BitsAlias.STR_STR,TypeStruct.STR_STR); }
  @Override public Node ideal(GVNGCM gvn) { return null; }
  @Override public Type value(GVNGCM gvn) {
